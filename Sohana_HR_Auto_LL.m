...
newY = new_Y(:,1);
mdl = fitglm(znewX', newY',  'distr', 'binomial', 'link', 'logit')

p=mdl.Fitted.Response;
z=mdl.Fitted.LinearPredictor;
figure, gscatter (z, p, newY, 'br'); grid on 
%plotSlice (mdl)
...
outlier = find((mdl.Diagnostics.CooksDistance)>5*mean(mdl.Diagnostics.CooksDistance));

mdl1 = fitlm(znewX', newY', 'Exclude',outlier);
mdl1.ObservationInfo(outlier,:);
...
[B,FitInfo] = lassoglm(znewX2, new_Y,'binomial',...
'NumLambda',15,'PredictorNames',....
...
PredictedClasses = double(predict(mdl1, znewX') >= 0.5);
...
%roc for class 1
probability1 = predict(mdl1, znewX');
